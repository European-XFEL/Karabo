...
- Curve Options: Customize pen colors and legend names for each curve
...
- Karabo Projects:
  ...
  - More convenience in adding servers
...
- Device Properties Removed: Properties like `_serverId` and `_deviceId` have been removed.
...
- Class Changes:
  ...
  - `Trainstamp` (C++/Bound) has been removed; use `TimeId`.
...
- MDL:
  ...
  - `EncodingType` has been renamed to `Encoding`.
...
can specify USE_LEGACY_PYTEST = "false" to make use of this feature. After that, the legacy behavior changes its default.
