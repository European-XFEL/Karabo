...
Release date will be on November 13 2023.
...
- GUI: **LogBook support!** Scenes, configs, etc. with annotation can be send to
  an electronic log book backend. Device was developed for Zulip ELOG at EuXFEL.
- GUI: A new application mode is provided, the **karabo-concert**.
  From a running GUI, a yaml file can be created that stores information about
  open scenes and their position on the screen.
  If this yaml file is passed to karabo-concert, scenes are positioned accordingly.
- GUI: XY Plots now gracefully accept vectors of different length
- All API: First official production release of the Rabbit MQ (AMQP) interface.
- C++: Device servers support now an `init` configuration (json) similar to existing MDL for autostarting devices.
- Bound: updateState can be called now with extra arguments for a single update
- Python API: All device templates are now provided with a toml structure.
  The framework supports and is now aligned to PEP0621 (https://peps.python.org/pep-0621/)
  All device tags must follow PEP440 (https://peps.python.org/pep-0440/)
- C++/Bound: The pipelining interface was enhanced and is now asynchronous under the hood.
- All API: slotGetSystemInfo implementation for devices to provide information about their broker connection,
...
- C++ ImageData header has been removed, see migration guidelines.
- C++/Bound: DetectorGeometry has been removed, see migration guidelines.
- C++: Removal of the Motor and Camera interface class.
- C++: Remove 'disributionMode' from OutputChannel behaviour for shared connections. The default behaviour is unchanged (load-balanced).
- C++/Bound: As a more powerful feature, a device can register a method to its output channel that decides which of the shared input channels should receive data.
...
- AlarmService: The alarm service has been removed from the framework. A new alarm device implementation will be provided outside of the frameowrk.
...
- All API: The pipeline option `queue` has been deprecated and treated as `queueDrop` if configured.
...
- C++: Output channel schema default values are not included anymore in the device configuration. This is visible when calling
