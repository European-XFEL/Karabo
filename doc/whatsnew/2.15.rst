...
This Karabo release contains against a ``nympy`` dependency update: **1.22.3**
...
- C++/Bound: Both C++ and Bound devices will shutdown if they experience an exception in their `initialization` method.
...
- C++: Fix implicit default value of readOnly tableFixes
- C++ influx: Do not wait for connection with mutex lockedPlus minor clean-up like less handler copies.
- C++: Protected signal container handling (and remove dead code)Inspired from to https://in.xfel.eu/redmine/issues/121483, some missing mutex protections have been identified. But it is not deemed likely that they fix the issue.
...
- Bound: Raise on inconsistent NDArray in Hash
- C++/Bound: Better device instantiation failure reply. Split short message and details for device instantiation failure.
...
- GuiServer: Gui server keeps registered pipelines
- GuiServer: Increase minimum client version to 2.11.3 !6295
- GuiServer: More pipeline info in GuiServer debug dump.
- GuiServer: Print meta data received from client !6541
- GuiServer: GuiServerDevice synchronisation fix. Register device monitoring with lock to ensure to stay in sync with `m_monitoredDevices`.
...
- C++ tests: Safer TcpAdapter with extended login(..)method. Use of bind_weak protects against failures like "try to lock invalid mutex" that were at least seen in failure conditions.
