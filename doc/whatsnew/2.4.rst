...
Bound/cpp core
++++++++++++++
- Python Karathon SignalSlotable Type Handle Test !3038
...
- C++: No unordered_map if we erase iterators while looping on map !3299
- C++: Set custom display type for nodes !3273
- C++ OutputChannel: Single queue for load-balanced shared inputs. !3100
- More logs and few fixes for p2p shortcut !3331
- Epochstamp: Add operator<< for output to ostream !3077
...
- Agressively avoid one copy when forwarding pipeline data to GUI !3322
- GUISERVER: allow the server to go as fast as possible !3340
...
- GUI Server: Use minInc for time intervals !3301
- Gui servers treats completely failing instantiate request !3145
- DataLogger: 'Fix history retrieval (RedMine 13865) !3249
- FEAT: path elements in the Property Test !3258
- GUI Server: Cache and forward logs !3204
- RunConfigurationGroup: Table combobox !3213
- Core: Move often occuring logs in Device.hh to debug !3220
- GUI SERVER: loglevel default to INFO !3217
...
- CI: Added test coverage job on gitlab-ci !2885
