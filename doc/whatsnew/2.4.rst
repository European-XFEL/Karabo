...
In addition, for bound python the global interpreter lock is released for many additional functions in the deviceclient
when calling c++ code.
...
Bound: Release GIL when connecting to a signal (for 2.4.1)When a (potentially) blocking C++ function is bound to Python, we should
release the global interpreter lock before. !3463
Fix pipeline integration test wrt changed remoteId !3434

Middlelayer core
++++++++++++++++

MDL: Send channel name in input channel for tracking (2.4.1)Cheery picked Karabo/Framework!3404 !3432
MDL: Track channel connections in output channel (for 2.4.X)Cherry picked from master (4d407473) !3420
