...
void exportPyUtilStateElement() {
    bp::implicitly_convertible< Schema &, StateElement >();
        bp::class_<StateElement> ("STATE_ELEMENT", bp::init<Schema & >((bp::arg("expected"))))
                .def("alias"
                     , &AliasAttributeWrap<StateElement>::aliasPy
                     , bp::return_internal_reference<> ())
                .def("commit", &StateElement::commit
                     , bp::return_internal_reference<> ())
                .def("commit"
                     , (StateElement & (StateElement::*)(karabo::util::Schema &))(&StateElement::commit)
                     , bp::arg("expected")
                     , bp::return_internal_reference<> ())
                .def("description", &StateElement::description
                     , bp::return_internal_reference<> ())
                .def("displayedName", &StateElement::displayedName
                     , bp::return_internal_reference<> ())
                .def("key", &StateElement::key
                     , bp::return_internal_reference<> ())
                .def("tags"
                     ...
                     , bp::return_internal_reference<> ())
                .def("tags"
                     , (StateElement & (StateElement::*)(std::vector<std::string> const &))(&StateElement::tags)
                     , (bp::arg("tags"))
                     , bp::return_internal_reference<> ())
                .def("options", bp::raw_function(&StateElementWrap::optionsPy,2))
                .def("daqPolicy", &StateElement::daqPolicy
                     , bp::return_internal_reference<> ())
                .def("defaultValue", &StateElementWrap::initialValuePy
                     , (bp::arg("value"))
                     , bp::return_internal_reference<> ())
