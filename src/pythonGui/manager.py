...
### slots ###
    ...
    def onOpenHashFromXml(self):
        filename = QFileDialog.getOpenFileName(self, "Open Saved Configuration", QDir.tempPath(), "XML (*.xml)")
        if filename.isEmpty() :
            return


    def onSaveHashToXml(self):
        filename = QFileDialog.getSaveFileName(self, "Save File", QDir.tempPath(), "XML (*.xml)")
        if filename.isEmpty() :
            return
