...
from karabogui.sceneview.bases import BaseSceneTool, BaseSceneAction
from karabogui.sceneview.workflow.api import (
...
def connect_channels(start_channel, end_channel):
    ...
    device = end_channel.proxy.root_proxy
    binding = end_channel.proxy.binding
    if binding is None or not isinstance(device, ProjectDeviceProxy):
        # `binding` is None if the schema hasn't arrived for the project device
        # and if `device` is _not_ a ProjectDeviceProxy, it means the real
        # device is online, and the connections can't be modified.
        return
...
def disconnect_channels(start_channel, end_channel):
    ...
    binding = end_channel.proxy.binding
    if binding is None or not isinstance(device, ProjectDeviceProxy):
        # ^See above in `connect_channels` for what is being checked here^
        return
