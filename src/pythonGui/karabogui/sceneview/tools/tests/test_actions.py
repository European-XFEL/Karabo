...
@pytest.fixture()
def scene_view(gui_app):
    # Initialize the labels for the SceneModel
...
def test_basics(scene_view):
    ...
    _assert_sceneview_contents(scene_view, changed=False)


@pytest.mark.skipif(IS_MAC_SYSTEM, reason="Segfault on MacOS")
def test_horz_layout_action(scene_view):
    ...
    action.perform(scene_view)
    _assert_sceneview_contents(scene_view, changed=True)
...
@pytest.mark.skipif(IS_MAC_SYSTEM, reason="Segfault on MacOS")
def test_vert_layout_action(scene_view):
...
def _assert_sceneview_contents(view, changed=True):
...
def _assert_layout_in_scene(view, model_klass=None, valid=True):
    ...
    assert layout_model is not None if valid else layout_model is None
