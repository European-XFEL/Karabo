...
public:
	...
	Hash config("brokers", fromString<std::string, std::vector>(m_brokers), "domain", m_domain, "instanceId",m_instanceId);
...
private:
	...
	if (msg) {
		...
		CPPUNIT_ASSERT_MESSAGE("*** disconnectAsync : client is still connected", m_client->isConnected() == false);
		m_promise.set_value();
	    });
	}
    }
...
void MqttClient_Test::testConnectSync_(const std::string& classId) {
    ...
    Hash config(
        ...
        "domain",     m_domain,
        "instanceId", "testConnectSync_" + classId
    );
    ...
    config.set("brokers", fromString<std::string, std::vector>(std::string("mqtt://invalidbroker.example.com:7777,") +  default_broker));
    client = MqttClient::create(classId, config);
    ec = client->connect();
    CPPUNIT_ASSERT(ec == KARABO_ERROR_CODE_SUCCESS);
    CPPUNIT_ASSERT(client->isConnected() == true);
...
void MqttClient_Test::testConnectAsync_(const std::string& classId) {
    ...
    Hash config("brokers", fromString<std::string, std::vector>(m_brokers),
                "domain", m_domain, "instanceId","testConnectAsync_"+classId);
    ...
    config.set("brokers",
               fromString<std::string, std::vector>(
                    std::string("mqtt://invalidbroker.example.com:7777,") +
                    default_broker));
    {
        std::promise<void> p;
        auto f = p.get_future();
...
void MqttClient_Test::testTryingToCallOperationsWithoutBeingConnected_(const std::string& classId)
{
    ...
    Hash config("brokers", fromString<std::string, std::vector>(m_brokers), "domain", m_domain, "instanceId","testTryingToCallOperationsWithoutBeingConnected");
    auto client = MqttClient::create(classId, config);
...
void MqttClient_Test::testPublishSubscribeSync(const std::string& classId, const int qos) {
    ...
    // Used for synchronization (http://www.modernescpp.com/index.php/thread-synchronization-with-condition-variables-or-tasks)
    std::promise<void> promise;
    auto future = promise.get_future();

    std::string instanceId = "test" + classId + operationStr;
    Hash config("brokers", fromString<std::string, std::vector>(m_brokers), "domain", m_domain, "instanceId",instanceId);
    auto client = MqttClient::create(classId, config);
    auto ec = client->connect();        
    CPPUNIT_ASSERT(!ec);
...
void MqttClient_Test::testPublishMultiSubscribeSync(const std::string& classId, const int qos) {
    ...
    topics.push_back(std::make_tuple(TOPIC1,
                                     subopts,
                                     [&]
                                     (boost::system::error_code ec,
                                      const std::string topic,
    ...
    topics.push_back(std::make_tuple(TOPIC2,
                                     subopts,
                                     [&]
                                     (boost::system::error_code ec,
                                      const std::string topic,
...
void MqttClient_Test::testPublishManySubscribeAsync(const std::string& classId, const int qos) {
    ...
    const std::string SPEC   = m_domain + "/" + classId + "/testPublishManySubscribeAsync_SPECIAL_" + operationStr + "_Async/";
        ...
        (const boost::system::error_code ec) {
            ...
            client->subscribeAsync(
                ...
                (const boost::system::error_code& ec) {
                    ...
                    Hash::Pointer msg(new Hash("header",
                                               Hash("header", "some header"),
                                               "body", Hash("body", 77)));
...
void MqttClient_Test::testMultipleSubscribersToTheSameTopic_(const std::string& classId) {
	...
	karabo::util::Hash::Pointer produceMessage() {
		...
		msg = boost::make_shared<Hash>("header", Hash("instanceId", m_instanceId), 
			"body", Hash("messageId", ++m_numMsgsSent));
	    }
	    return msg;
	}
    private:
...
void MqttClient_Test::testMultipleSubscriptionsToTopicWithWildcardsAndSubtopics_(const std::string& classId) {
    ...
    auto readConsume1 = [&] (const boost::system::error_code& rc,
                             const std::string& topic,
    ...
    auto readConsume2 = [&] (const boost::system::error_code& rc,
                             const std::string& topic,
                             const karabo::util::Hash::Pointer& hash) {
        if (rc) return;
        ++n2;
...
void MqttClient_Test::testTopicsSubscriptionsInArbitraryOrder_(const std::string& classId) {
    ...
    public:
        ...
        void start(const std::string& classId) {
            ...
            m_client->connectAsync([this] (const boost::system::error_code ec) {
                ...
                params.push_back(std::make_tuple(m_topic1, subopts, boost::bind(&Consumer::handleRead1,this,_1,_2,_3)));
                params.push_back(std::make_tuple(m_topic0, subopts, boost::bind(&Consumer::handleRead0,this,_1,_2,_3)));
                params.push_back(std::make_tuple(m_topic2, subopts, boost::bind(&Consumer::handleRead2,this,_1,_2,_3)));
                m_client->subscribeAsync(params,
                                         [this]
                                         (const boost::system::error_code ec) {
                                                CPPUNIT_ASSERT(!ec);
        ...
        boost::system::error_code subscribe(const std::vector<std::tuple<std::string,SubOpts,ReadHashHandler>>& topics) {
            return m_client->subscribe(topics);
        }
    ...
    private:
        ...
        void handleRead0(boost::system::error_code ec, const std::string& topic, const karabo::util::Hash::Pointer msg) {
            throw KARABO_LOGIC_EXCEPTION("Wildcard handler should NOT be called! Topic: \"" + topic + "\"");
        }

        void handleRead1(boost::system::error_code ec, const std::string& topic, const karabo::util::Hash::Pointer msg) {
            if (ec) return;
            ++m_n1;
            if (m_n1 == NUM_MSGS_PER_PUBLISHER) m_promise1.set_value();
        }

        void handleRead2(boost::system::error_code ec, const std::string& topic, const karabo::util::Hash::Pointer msg) {
            if (ec) return;
            ++m_n2;
