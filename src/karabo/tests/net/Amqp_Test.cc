...
void Amqp_Test::testClientTooBigMessage() {
    ...
    net::AmqpClient2::ReadHandler readerBob([&readMsg](const std::shared_ptr<std::vector<char>>& data,
                                                       const std::string& exch,
                                                       const std::string& key) { readMsg = data; });
    net::AmqpClient2::Pointer bob(
          boost::make_shared<net::AmqpClient2>(connection, prefix + "bob", AMQP::Table(), readerBob));
    net::AmqpClient2::Pointer alice(
          boost::make_shared<net::AmqpClient2>(connection, prefix + "alice", AMQP::Table(),
                                               net::AmqpClient2::ReadHandler())); // alice won't read
    ...
    const size_t supportedSize = net::AmqpClient2::m_maxMessageSize; //  134'217'728ul;
    ...
    // Now manipulate AmqpClient2 to not catch a too big message - but let broker complain.
    ...
    net::AmqpClient2::m_maxMessageSize += 1;
    pubPromise = std::make_shared<std::promise<boost::system::error_code>>();
    pubFuture = pubPromise->get_future();
    ...
    // responding log message from AmqpClient2::channelErrorhandler(..).
