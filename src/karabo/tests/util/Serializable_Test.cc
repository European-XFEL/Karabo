...
void Serializable_Test::testMethod() {    
    ...
    FancyData::Pointer fd1(new FancyData());
    fd1->setScalar(1);
    ...
    Hash h;
    h.set("fd1", fd1); // This will use the shared_ptr, without copying
    h.set("fd2", fd2); // Here the original object on the stack is copied

    // Internally all objects are hold as shared_ptrs
    CPPUNIT_ASSERT(h.get<FancyData>("fd1").getScalar() == 1);
    CPPUNIT_ASSERT(h.get<FancyData::Pointer>("fd1")->getScalar() == 1);
    CPPUNIT_ASSERT(h.get<FancyData>("fd2").getScalar() == 2);
    CPPUNIT_ASSERT(h.get<FancyData::Pointer>("fd2")->getScalar() == 2);

    // As r-values are references this modifies the shared_ptr object
    h.get<FancyData>("fd1").setScalar(-1);
    ...
    // Here a copy is done
    FancyData fd3 = h.get<FancyData>("fd1");
    fd3.setScalar(1);
