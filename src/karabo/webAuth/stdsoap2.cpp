...
zlib_again:
        ...
        if ((soap->mode & SOAP_IO) == SOAP_IO_CHUNK) /* read HTTP chunked transfer */ {
            for (;;) {
                ...
                do
                    *t++ = (char) c; while (soap_isxdigit((int) (c = soap_getchunkchar(soap))) && (size_t) (t - tmp) < sizeof (tmp) - 1);
                while ((int) c != EOF && c != '\n')
    ...
    soap_get(struct soap *soap) {
        ...
        while ((int) c != EOF) {
            ...
            switch (c) {
                    case '<':
                    do c = soap_get1(soap); while (soap_blank(c));
                    if (c == '!' || c == '?' || c == '%') {
                        register int k = 1;
                        if (c == '!') {
                            c = soap_get1(soap);
                            if (c == '[') {
                                do c = soap_get1(soap); while ((int) c != EOF && c != '[');
                                if ((int) c == EOF)
                                    break;
                                soap->cdata = 1;
                                c = soap_get1(soap);
    ...
    int
    SOAP_FMAC2
    soap_xop_forward(struct soap *soap, unsigned char **ptr, int *size, char **id, char **type, char **options) { /* Check MTOM xop:Include element (within hex/base64Binary) */
    ...
    void
    SOAP_FMAC2
    soap_ssl_init() { /* Note: for MT systems, the main program MUST call soap_ssl_init() before any threads are started */
        if (!soap_ssl_init_done) {
...
again:
    ...
    tcp_select(struct soap *soap, SOAP_SOCKET sk, int flags, int timeout) {
            ...
            #ifdef HAVE_POLL
        {
            ...
            do r = poll(&pollfd, 1, timeout); while (r == 0 && retries--);
            if (r > 0) {
                r = 0;
        ...
        if (soap->status == 0) {
            ...
            if (s && httpcmd) {
                ...
                if (httpcmd > 1) {
                    ...
                    switch (httpcmd) {
                            case 2: soap->error = soap->fput(soap);
                            break;
                        case 3: soap->error = soap->fget(soap);
                            break;
                        case 4: soap->error = soap->fdel(soap);
                            break;
                        case 5: soap->error = soap->fopt(soap);
                            break;
                        case 6: soap->error = soap->fhead(soap);
                            break;
                        default: soap->error = 405;
                            break;
                    }
...
/******************************************************************************/
        ...
        while ((int) c != EOF && c != '>' && c != '/') {
            ...
            *s = '\0';
            ...
            while (soap_blank(c))
                c = soap_get1(soap);
            if (c == '=') {
                do c = soap_getutf8(soap); while (soap_blank(c));
                    ...
                    switch (c) {
                        ...
                        case '<':
                            ...
                            if (c == '/') {
                                ...
                                c = soap_getchar(soap);
                                if (c == '[') {
                                    do c = soap_getchar(soap); while ((int) c != EOF && c != '[');
                                    if ((int) c == EOF)
                                        goto end;
                                    t = (char*) "![CDATA[";
                                    m = 8;
    ...
    soap_value(struct soap *soap) {
        ...
        if (!soap->body)
            return SOAP_STR_EOS;
        do c = soap_get(soap); while (soap_blank(c));
                ...
                if (m > 0) {
                    *s++ = *t++;
                    ...
                    if (flag || c == '\r') {
                        ...
                        do c = soap_getchar(soap); while (c == *t++);
                        if ((int) c == EOF) {
                            if (content->ptr && soap->fmimewriteclose)
                                soap->fmimewriteclose(soap, (void*) content->ptr);
                            soap->error = SOAP_CHK_EOF;
    ...
    static int
    soap_ntlm_handshake(struct soap *soap, int command, const char *endpoint, const char *host, int port) { /* requires libntlm from http://www.nongnu.org/libntlm/ */
