...
namespace karabo {
    namespace io {
        namespace h5 {
            ...
            File::File(const std::string& filename) : m_filename(filename), m_h5file(-1), m_managed(false) {}
            ...
            void File::open(File::AccessMode mode) {
                ...
                try {
                    if (mode == TRUNCATE) {
                        ...
                        //                        m_h5file = H5Fcreate(m_filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                        //                        fip);
            ...
            void File::close() {
                KARABO_LOG_FRAMEWORK_TRACE_CF << "start closing file " << m_filename.string()
                                              << " Num. open tables: " << m_openTables.size();
                if (!isOpen()) return;
