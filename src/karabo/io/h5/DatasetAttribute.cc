    ...
    namespace io {
        namespace h5 {
            ...
            void DatasetAttribute::write(const karabo::util::Hash& data, hsize_t recordId) {
                ...
                KARABO_LOG_FRAMEWORK_TRACE_C("karabo.io.h5.DatasetAttribute") << "Writing hash data: key=" << m_key <<
            ...
            void DatasetAttribute::write(const karabo::util::Hash& data, hsize_t recordId, hsize_t len) {
                KARABO_LOG_FRAMEWORK_TRACE_C("karabo.io.h5.DatasetAttribute") << "Writing hash data: key=" << m_key << " recordId=" << recordId << " len=" << len;
