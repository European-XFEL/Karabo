    ...
    namespace util {
        ...
        class DetectorGeometry : public boost::enable_shared_from_this<DetectorGeometry> {
            ...
            DetectorGeometry() : m_offsets(std::vector<double>(3, 0.)), m_rotations(std::vector<double>(3, 0.)),
                m_tileId(-2), m_parent(NULL), m_managedTiles(0), m_pixelRegion(std::vector<int>(4, -1)) {
            ...
            DetectorGeometry& setPixelRegion(const unsigned int & x0, const unsigned int & y0, const unsigned int & x1, const unsigned int & y1) {
                m_pixelRegion[0] = x0;
                m_pixelRegion[1] = y0;
                m_pixelRegion[2] = x1;
                m_pixelRegion[3] = y1;
            ...
            karabo::util::Schema toSchema(std::string topNode, karabo::util::Schema & schema, bool topMost = true) {
                ...
                karabo::util::NODE_ELEMENT(schema).key(topNode)
                        .commit();

                //}
                karabo::util::BOOL_ELEMENT(schema).key(topNode + ".update")
                ...
                if (m_pixelRegion[0] != -1) {
                    karabo::util::VECTOR_INT32_ELEMENT(schema).key(topNode + ".pixelRegion")
                ...
                for (std::vector<DetectorGeometry>::iterator it = m_subAssemblies.begin(); it != m_subAssemblies.end(); ++it) {
                    karabo::util::NODE_ELEMENT(schema).key(topNode + ".t" + boost::lexical_cast<std::string>(it->getTileId()))
                            ...
                            .commit();
                    schema.merge(it->toSchema(topNode + ".t" + boost::lexical_cast<std::string>(it->getTileId()), schema, false));
                    idOnHierarchy++;
                }
