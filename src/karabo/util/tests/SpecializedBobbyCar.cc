...
namespace exfel {
  namespace util {
    ...
    //SpecializedBobbyCar::SpecializedBobbyCar() : BobbyCar(this) {
    //}

    void SpecializedBobbyCar::start() {
      cout << "Starting " << m_color << " " << m_name << ", turning " << m_equipment << " on" << endl;
    }

    void SpecializedBobbyCar::stop() {
      cout << "Stopping " << m_color << " " << m_name << ", turning " << m_equipment << " off" << endl;
    }

    void SpecializedBobbyCar::expectedParameters(Schema& expected) {

      STRING_ELEMENT(expected).key("name")
              .displayedName("Brand")
              .description("Brand of the BobbyCar")
              .options("Summer,Winter,ThisSummer")
              .assignmentMandatory()
              .reconfigurable()
              .commit();
    ...
    void SpecializedBobbyCar::configure(const Hash& conf) {
    ...
    //EXFEL_REGISTER_FACTORY_2_CC(Vehicle, BobbyCar, SpecializedBobbyCar)
