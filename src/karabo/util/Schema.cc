...
namespace karabo {
    namespace util {
        ...
        //            set("root", key);
        //            Schema tmp(m_currentAccessMode, m_currentState);
        //            set("elements", tmp);
        //            return get<Schema > ("elements");
        //        }

        Schema::Schema(const std::string& classId) : m_injectDefaults(true), m_allowUnrootedConfiguration(false),
        m_allowAdditionalKeys(false), m_allowMissingKeys(false), m_currentAccessMode(WRITE | INIT),
        m_currentState(""), m_currentAccessRole("") {

            this->setRoot(classId);
        }
        
        void Schema::setRoot(const std::string& rootName) {
            m_hash.set(rootName, Hash()).setAttribute("nodeType", "root");
        }
        ...
        void Schema::filteringSetAccessMode(const AccessType& currentAccess) {
            m_currentAccessMode = currentAccess;
        }
        
        void Schema::filteringSetAccessRole(const std::string& currentAccessRole) {
            m_currentAccessRole = currentAccessRole;
        }
        
        void Schema::filteringSetState(const std::string& currentState) {
            m_currentState = currentState;
        }
        
        const AccessType& Schema::filteringGetAccessMode() const {
            return m_currentAccessMode;
        }
        
        const std::string& Schema::filteringGetAccessRole() const {
            return m_currentAccessRole;
        }
        
        const std::string& Schema::filteringGetState() const {
            return m_currentState;
        }
        
        void Schema::validationInjectDefaults(const bool toggle) {
            m_injectDefaults = toggle;
        }
        
        void Schema::validationAllowUnrootedConfiguration(const bool toggle) {
            m_allowUnrootedConfiguration = toggle;
        }
        
        void Schema::validatationAllowAdditionalKeys(const bool toggle) {
            m_allowAdditionalKeys = toggle;
        }
        
        void Schema::validatationAllowMissingKeys(const bool toggle) {
            m_allowMissingKeys = toggle;
        }
        ...
        void Schema::ensureParameterDescriptionIsComplete(Hash::Node& node) const {
            std::string error;
            //if (!node.hasAttribute("nodeType")) error = "Missing nodeType attribute";            
            
            if (!error.empty()) throw KARABO_PARAMETER_EXCEPTION(error);
        }
...
//        Schema& Schema::addParameter(Schema& item) {
//
...
//            if (!(m_currentAccessMode & item.get<AccessType > ("access"))) return *this;
//
...
//            if (item.has("complexType")) item.setAccessMode(m_currentAccessMode);
//
