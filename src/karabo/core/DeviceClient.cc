...
#define KARABO_IF_SIGNAL_SLOTABLE_EXPIRED_THEN_RETURN(...) if (m_signalSlotable.expired()) { \
        ...
        void DeviceClient::_slotInstanceNew(const std::string& instanceId, const karabo::util::Hash& instanceInfo) {
            ...
            if (m_loggerMapCached && instanceId == devices::DATALOGMANAGER_ID) {
                karabo::xms::SignalSlotable::Pointer p = m_signalSlotable.lock();
                if (p) {
                    ...
                    p->requestNoWait(devices::DATALOGMANAGER_ID, "slotGetLoggerMap", "", "_slotLoggerMap");
                }
            }
        }
        ...
        bool DeviceClient::cacheLoggerMap(bool toggle) {
            ...
            if (!p) {
                ...
                if (p->connect(devices::DATALOGMANAGER_ID, "signalLoggerMap", "", "_slotLoggerMap")) {
                    // If we cannot connect, request makes no sense
                    Hash loggerMap;
                    try {
                        p->request(devices::DATALOGMANAGER_ID, "slotGetLoggerMap").timeout(m_internalTimeout).receive(loggerMap);
                ...
                if (!p->disconnect(devices::DATALOGMANAGER_ID, "signalLoggerMap", "", "_slotLoggerMap")) {
        ...
        std::string DeviceClient::getDataLogReader(const std::string& deviceId) {
            ...
            const std::string loggerId(devices::DATALOGGER_PREFIX + deviceId);
            ...
            if (dataLogServer.empty()) {
                ...
                if (p) {
                    ...
                    try {
                        p->request(devices::DATALOGMANAGER_ID, "slotGetLoggerMap").timeout(m_internalTimeout).receive(localLogMap);
                    } catch (const TimeoutException&) {
            ...
            if (dataLogServer.empty()) {
                ...
                (dataLogReader += devices::DATALOGREADER_PREFIX) += toString(i++ % devices::DATALOGREADERS_PER_SERVER)
                        += "-" + dataLogServer;
            }
