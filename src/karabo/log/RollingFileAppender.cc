...
namespace karabo {
    namespace log {
        ...
        void RollingFileAppender::expectedParameters(Schema& s) {
            ...
            STRING_ELEMENT(s).key("category")
                    .description("Category")
                    .displayedName("Category")
                    .assignmentOptional().defaultValue("")
                    .commit();

            PATH_ELEMENT(s).key("filename")
            ...
            UINT32_ELEMENT(s).key("maxFileSize")
                    .description("Maximum file size")
                    .displayedName("MaxFileSize")
                    .unit(Unit::BYTE)
                    .assignmentOptional().defaultValue(10 * 1024 * 1024)
                    .commit();

            UINT32_ELEMENT(s).key("maxBackupIndex")
            ...
            STRING_ELEMENT(s).key("pattern")
                    ...
                    .assignmentOptional().defaultValue("%d{%F %H:%M:%S} %p  %c  : %m%n")
                    .commit();
        ...
        RollingFileAppender::RollingFileAppender(const Hash& input) {
            ...
            m_appender = new krb_log4cpp::RollingFileAppender("file",
                                                              input.get<string>("filename"),
