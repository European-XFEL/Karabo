    ...
    namespace log {
        ...
        void Log4CppNetApp::checkLogCache(const boost::system::error_code& e) {
            ...
            try {
                ...
                boost::mutex::scoped_lock lock(m_mutex);
                KARABO_LOG_FRAMEWORK_ERROR << "Writing failed for " << m_logCache.size() << " message(s): " << e;
        ...
        void Log4CppNetApp::writeNow() {
            ...
            if (m_logCache.size() > m_maxMessages) {
                KARABO_LOG_FRAMEWORK_ERROR << "Send only maximum of " << m_maxMessages << " messages instead of the "
                                           << m_logCache.size() << " accumulated ones.";
