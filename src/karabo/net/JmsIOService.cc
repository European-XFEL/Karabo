...
namespace exfel {
    namespace net {
        ...
        //EXFEL_REGISTER_FACTORY_CC(AbstractIOService, JmsIOService)
        ...
        bool JmsIOService::activateRegisteredTextMessageHandlers() {
            ...
            for (size_t i = 0; i < m_textMessageChannels.size(); ++i) {
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::listenForTextMessages, m_textMessageChannels[i]));
            }
        ...
        bool JmsIOService::activateRegisteredBinaryMessageHandlers() {
            ...
            for (size_t i = 0; i < m_binaryMessageChannels.size(); ++i) {
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::listenForBinaryMessages, m_binaryMessageChannels[i]));
            }
        ...
        bool JmsIOService::activateRegisteredWaitHandlers() {
            ...
            for (size_t i = 0; i < m_waitHandlers.size(); ++i) {
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::deadlineTimer, m_waitHandlers[i].get<0>(), m_waitHandlers[i].get<1>(), m_waitHandlers[i].get<2>()));
            }
        ...
        void JmsIOService::registerTextMessageChannel(JmsChannel* channel) {
            ...
            if (m_status == IDLE || m_status == STOPPED || m_status == RUNNING) {
                ...
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::listenForTextMessages, channel));
            }
        }
        ...
        void JmsIOService::registerBinaryMessageChannel(JmsChannel* channel) {
            ...
            if (m_status == IDLE || m_status == STOPPED || m_status == RUNNING) {
                ...
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::listenForBinaryMessages, channel));
            }
        }
        ...
        void JmsIOService::registerWaitChannel(JmsChannel* channel, const WaitHandler& handler, int milliseconds) {
            ...
            if (m_status == IDLE || m_status == STOPPED || m_status == RUNNING) {
                ...
                m_threadGroup.create_thread(boost::bind(&exfel::net::JmsChannel::deadlineTimer, channel, handler, milliseconds));
            }
        }
    }
}
