...
namespace karabo {
    namespace net {
        ...
        AmqpBroker::AmqpBroker(const AmqpBroker& o, const std::string& newInstanceId)
            ...
            , m_client(Configurator<AmqpClient>::create(AMQP_CLIENT_CLASS,
        ...
        void AmqpBroker::connect() {
            if (!m_client->isConnected()) {
                ...
                if (ec) {
                    ...
                    oss << "Failed to connect to AMQP broker: code #"
                            << ec.value() << " -- " << ec.message();
        ...
        void AmqpBroker::disconnect() {
            ...
            if (m_heartbeatConsumerChannel && m_heartbeatConsumerChannel->isConnected()) m_heartbeatConsumerChannel->disconnect();
        ...
        void AmqpBroker::amqpReadHashHandler(const boost::system::error_code& ec,
            ...
            if (m_client && m_client->isConnected() && !ec) {
                ...
                // Error ...
                std::ostringstream oss;
                oss << "Exchange -> \"" << exchange << "\", routing key -> \""
                        << routingkey << "\" : Error code #" << ec.value()
                        << " -- " << ec.message();
                if (errorNotifier) {
        ...
        boost::system::error_code AmqpBroker::subscribeToRemoteSignal(const std::string& signalInstanceId,
            ...
            subscribeToRemoteSignalAsync(signalInstanceId, signalFunction, [prom](const boost::system::error_code & ec) {
                prom->set_value(ec);
            });
            return fut.get();
        }
        ...
        void AmqpBroker::subscribeToRemoteSignalAsync(const std::string& signalInstanceId, const std::string& signalFunction,
        ...
        boost::system::error_code AmqpBroker::unsubscribeFromRemoteSignal(const std::string& signalInstanceId,
            ...
            auto prom = std::make_shared<std::promise<boost::system::error_code> >();
            auto fut = prom->get_future();
            unsubscribeFromRemoteSignalAsync(signalInstanceId, signalFunction, [prom](const boost::system::error_code & ec) {
                prom->set_value(ec);
            });
            return fut.get();
        }
        ...
        void AmqpBroker::write(const std::string& target,
        ...
        void AmqpBroker::publish(const std::string& exchange,
        ...
        void AmqpBroker::startReading(const consumer::MessageHandler& handler,
            ...
            auto readHandler = bind_weak(&AmqpBroker::amqpReadHashHandler, this, _1, _2, _3, _4, handler, errorNotifier);
            ...
            if (ec) {
                std::ostringstream oss;
                oss << "Subscription to exchange -> \"" << exchange
        ...
        void AmqpBroker::startReadingHeartbeats(const consumer::MessageHandler& handler,
            ...
            if (ec) {
                std::ostringstream oss;
                oss << "Failed to subscribe to exchange -> \"" << exchange << "\", bindingkey->\""
                        << bindingkey << "\" : code #" << ec.value() << " -- " << ec.message();
                throw KARABO_NETWORK_EXCEPTION(oss.str());
            }
        }
