...
namespace karabo {
    namespace net {
        ...
        public:
            ...
            void subscribeAsync(const TopicSubOptions& params,
        ...
        protected:
            ...
            boost::system::error_code subscribe(const std::string& topic,
                                                std::uint8_t subopts,
                                                const ReadHashHandler& onRead) override;

            void subscribeAsync(const std::string& topic, std::uint8_t subopts,
                                const ReadHashHandler& onRead,
                                const AsyncHandler& onComplete) override;

            boost::system::error_code publish(const std::string& topic,
        ...
        private:
            ...
            bool handleConnect(const bool sp,
                               const MQTT_NS::connect_return_code& rc,
            ...
            void handleSubscription(const boost::system::error_code& ec,
                                    const ReadHashHandler& onRead,
                                    const AsyncHandler& handler,
                                    const std::string& topic);

            void handleUnsubscription(const boost::system::error_code& ec,
                                      const AsyncHandler& handler,
                                      const std::string& topic);

            void registerManySubscriptions(const TopicSubOptions& params);

            void handleManySubscriptions(const boost::system::error_code& ec,
                                         const AsyncHandler& handler,
                                         const TopicSubOptions& params);

            void unregisterManySubscriptions(const std::vector<std::string>& topics);

            void handleManyUnsubscriptions(const boost::system::error_code& ec,
                                           const AsyncHandler& handler,
            ...
            bool handleMessage(MQTT_NS::optional<MqttNsClientPacketId> packetId,
            ...
            boost::optional<std::string> checkForMatchingOtherParams(const std::string& topic, const TopicSubOptions& params);
