...
namespace karabo::net {
    ...
    // Larger message create this error (seen in AmqpClient2::channelErrorHandler):
    ...
    size_t AmqpClient2::m_maxMessageSize = 134'217'728ul; // 128 MB
    ...
    void AmqpClient2::reviveIfReconnected() {
        ...
        if (numSubscriptions > 0) {
            ...
            auto singleSubscriptionDone =
                  [doPublishPostponed, doneFlags{std::make_shared<std::vector<bool>>(numSubscriptions, false)},
                   commonEc{std::make_shared<boost::system::error_code>(KARABO_ERROR_CODE_SUCCESS)},
            ...
            if (itExchange == m_exchanges.end()) { // e.g. connection lost,
                ...
                if (postponedMsg.data->size() > m_maxMessageSize) {
                    ...
                    KARABO_LOG_FRAMEWORK_ERROR
                          << "Dropping too big postponed message of size " << postponedMsg.data->size() << " instead "
                          << "of sending to " << postponedMsg.exchange << "." << postponedMsg.routingKey;
                      ...
                      .onSuccess([wSelf](const std::string& consumerTag) {
                          if (auto self = wSelf.lock()) {
                              ...
                              self->m_channel->onError(
                                    util::bind_weak(&AmqpClient2::channelErrorHandler, self.get(), _1));
                              AsyncHandler callback;
                              callback.swap(self->m_channelPreparationCallback);
                              callback(KARABO_ERROR_CODE_SUCCESS);
                          }
    ...
    void AmqpClient2::channelErrorHandler(const char* errMsg) {
