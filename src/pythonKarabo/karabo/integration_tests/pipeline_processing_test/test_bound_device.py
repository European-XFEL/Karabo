...
class TestPipelineProcessing(BoundDeviceTestCase):
    ...
    def test_in_sequence(self):
        ...
        with self.subTest(msg="Test Profile Transfer Times (short cut)"):
            self._profileTransferTimes(noShortCut=False)

        with self.subTest(msg="Test Profile Transfer Times (no short cut)"):
            self._profileTransferTimes(noShortCut=True)

    def _profileTransferTimes(self, noShortCut):
        receiver = "pipeTestReceiver"
        if noShortCut:
            os.environ["KARABO_NO_PIPELINE_SHORTCUT"] = "1"

            config = Hash("deviceId", "pipeTestReceiver2",
                          "processingTime", 0,
                          "input.connectedOutputChannels",
                          "p2pTestSender:output1",
                          "input2.connectedOutputChannels",
            ...
            state1 = None
            nTries = 0
            while state1 != State.NORMAL:
                try:
                    state1 = self.dc.get("pipeTestReceiver2", "state")
                # A RuntimeError will be raised up to device init
                except RuntimeError:
                    sleep(self._waitTime)
                    if nTries > self._retries:
                        raise RuntimeError(
                            "Waiting for device to init timed out")
                    nTries += 1
        ...
        self.assertTrue(self.waitUntilEqual(receiver, "nTotalData",
                                            132 - int(noShortCut) * 120,
                                            self.KRB_TEST_MAX_TIMEOUT))
        transferTime = self.dc.get(receiver, "averageTransferTime")*1000
        msg = "Average transfer time (copy, {}) is {} milliseconds"
        print(msg.format("no short cut" if noShortCut else "short cut",
                         transferTime))
        ...
        self.assertTrue(self.waitUntilEqual(receiver, "nTotalData",
                                            144 - int(noShortCut) * 120,
                                            self.KRB_TEST_MAX_TIMEOUT))

        transferTime = self.dc.get(receiver, "averageTransferTime")*1000
        msg = "Average transfer time (no copy, {}) is {} milliseconds"
        print(msg.format("no short cut" if noShortCut else "short cut",
                         transferTime))

        if noShortCut:
            os.environ["KARABO_NO_PIPELINE_SHORTCUT"] = ""
