...
class Broker(ABC):
    ...
    def create_connection(hosts, connection):
        ...
        if scheme == 'tcp':
            from karabo.middlelayer_api.jms_broker import JmsBroker
            return (JmsBroker.create_connection(hosts, connection),
                    JmsBroker)
        elif scheme == 'mqtt':
            from karabo.middlelayer_api.mqtt_broker import MqttBroker
            return (MqttBroker.create_connection(hosts, connection),
                    MqttBroker)
        elif scheme == 'redis':
            from karabo.middlelayer_api.redis_broker import RedisBroker
            return (RedisBroker.create_connection(hosts, connection),
                    RedisBroker)
        elif scheme == 'amqp':
            from karabo.middlelayer_api.amqp_broker import AmqpBroker
