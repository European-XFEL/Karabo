...
DEPENDENCIES_Linux_NoGui=( python3.4 setuptools pip wheel lapack numpy scipy nose libpng freetype pillow matplotlib pexpect tornado pyzmq pygments docutils sphinx ipython boost openmqc hdf5 h5py log4cpp cppunit openmq pyusb parse snappy jpeg httplib2 pssh tiff traits )
DEPENDENCIES_Darwin=( setuptools pip wheel pyqwt5 guidata guiqwt boost openmqc hdf5 h5py log4cpp cppunit parse snappy traits )
